...
const { dataVersion, setDataVersion } = useAppStore();
  ...
  const fetchData = async () => {
    try {
      ...
      const combined = [...expenses, ...stakingExpenses].sort((a, b) => {
    ...
    return (
        <div className="dashboard-container">
          <div className="dashboard-header">
            <h1 className="text-2xl font-bold text-gray-800">Track your Expenses on Solana and beyond!</h1>
...
{/*             
          ...
          {/* Button to add sample transactions */}
          <div className="flex justify-between items-center mb-4">
            <button 
              onClick={() => setDataVersion(dataVersion + 1)} 
            ...
            <div className="flex gap-2">
              {connected && publicKey && (
                <button 
                  onClick={addStakingEventForConnectedWallet} 
                  className="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-md flex items-center transition-colors duration-300"
                  disabled={isSeeding}
                >
                  <Plus className="mr-2 h-4 w-4" />
                  {isSeeding ? 'Adding...' : 'Add Solana Transaction'}
                </button>
              )}
